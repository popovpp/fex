...
class CustomUserAdmin(UserAdmin):
    ...
    model = User
    list_display = ('email', 'first_name', 'last_name', 'show_groups', 'is_active', 'is_staff', 'balance', 'freeze_balance')
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'password1', 'password2', 'first_name', 'last_name', 'groups', 'is_active')}
        ),
    )
    search_fields = ('email',)
    ordering = ('email',)
